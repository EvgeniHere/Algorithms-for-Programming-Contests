...
int main() {
    ...
    line ab_l = line(ab, cd);
    line ef_l = line(ef, gh);

    ray ab_r = ray(ab, cd);
    ray ef_r = ray(ef, gh);

    vec2 isect = intersect(ab_l, ef_l);

    lf lambdaA = dot(ab_r.b - ab_r.a, isect - ab_r.a) / norm_sqr(ab_r.b - ab_r.a);
    lf lambdaB = dot(ef_r.b - ef_r.a, isect - ef_r.a) / norm_sqr(ef_r.b - ef_r.a);

    if (parallel(ab_l, ef_l) || (lambdaA < 0. || lambdaB < 0)) {
        cout << min(dist(ef, project(ef, ab_r)), min(dist(gh, project(gh, ab_r)), min(dist(ab, project(ab, ef_r)), dist(cd, project(cd, ef_r))))) << endl;
    } else
        cout << 0. << endl;
